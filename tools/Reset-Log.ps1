...
{ 
    ...
    param([string]$fileName, [int64]$filesize , [int] $logcount) 
    ...
    { 
        ...
        { 
            ...
            {  
                ...
                $files = Get-ChildItem $filedir | ?{$_.name -like "$fn*"} | Sort-Object lastwritetime 
                $operatingFile = $files | ?{($_.name)-Split("$fn.") -eq $i} 
                if ($operatingfile) 
                 {$operatingFilenumber = ($files | ?{($_.name)-Split("$fn.") -eq $i}).name.trim($fn)} 
                else 
                ...
                { 
                    ...
                    $operatingFile = $files | ?{($_.name)-Split("$fn.") -eq ($i-1)} 
                ...
                { 
                    ...
                    {  
                        ...
                        $operatingFile = $files | ?{($_.name)-Split("$fn.") -eq $operatingFilenumber} 
