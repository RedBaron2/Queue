...
<#
    ...
    Version: 0.18.01.27d
    Updated KVRT to work with current development url as of 18-01-27
	Adjusted NPE helper to work with a switch and pass dwnld param
	KVRT will update if new filesize differs from local filesize
	(Note: ETag won't work for this file)
	
	Version: 0.18.2.2-d New Semver v1 versioning
    Adjustment to Queue Extraction call switch
    Added check of extArgs if [hashtable]
    Additional info added for KVRT/NPE files
    Adjustment to NPE_helpers for hashtable return
    Added more Diagnotic messages for Extraction
    Adjustment to wise365 $file to not be '' or null
	
#>
    $version = '0.18.5.9-c'
<#	
    Adjustment to Queue Extraction call switch for ddu
	Adjustments for crystal to work
...
function Get-FileFromWeb ( $Url, $filePath, $fileName ) {
	...
	$web_size = $fileName.web_size
	$local_size = $fileName.local_size
	posh-tee "queue web -$web_size- local -$local_size-"
	$fileName = $fn
	...
	posh-tee "Get-FileFromWeb $LocalPath"
		...
		posh-tee "website_version new -$website_version-"
			...
			switch -w ($fn) {
				...
				"DDU*" {
					posh-tee "Adjustment of Extraction output_dir"
					$neo_fn = $fn -replace('.exe','')
					posh-tee "#green#neo_fn# is #red#-$neo_fn-#"
					$extArgs = @{
						archive = "$LocalPath"
						output_dir = "${env:temp}\few"
				...
				"kvrt*" {
					$update = $true;
                    $website_version = $web_size;
                    $Extraction_version = $local_size;
				}
			...
			posh-tee "queue web -$web_size- local -$local_size-"
			if ($update) {
                if ( $extArgs -is [hashtable] ) {
                    posh-tee "We are off to run Extraction with -@extArgs-"
                    $Extraction_version = ( Extraction @extArgs )
                    posh-tee "Back from Extraction w/ -$Extraction_version-"
                }
         #       if ($extArgs) { clear-variable -name extArgs }
			...
			posh-tee "Extraction_version -$Extraction_version-"
				...
				posh-tee "Detected LocalPath -$LocalPath-"
					...
					posh-tee "Versions do not equal`n`r`
					This will require the deletion of -$LocalPath-"
					del $LocalPath
					posh-tee "We have del -$LocalPath-"
                    posh-tee "We need to download the file now dwnld -$dwnld-"
					} else {
                    $dwnld = $false;
                    }
				}
	...
	switch -w ( $fileName ) {
        ...
        'WiseCare365*.zip' {
            ...
            if (( $file -eq '' ) -or ( $file -eq $null )) { $file = $false; }
			$WCalt = "Wise_Alt_$me"
            posh-tee "The file is now -$file- WCalt -$WCalt- '($my_chcksums).$WCalt' -($my_chcksums).$WCalt-"
            $chksum = @{$true=(( Get-FileHash -Path $LocalPath ).SHA256);$false=( ($my_chcksums).$WCalt )}[( ($file) )]
            posh-tee "chksum -$chksum-"
            $test = @{$true=( $test );$false=(( Get-FileHash -Path $LocalPath ).SHA256)}[( ($file) )]
            posh-tee "$fileName NEW test -$test-"
            if ( $file ) {
                posh-tee "Starting wise365 file if -$file-"
                ...
                $WiseCareConfigTray = "[General]`rShowFloatWnd=0`r"
    ...
    #			( '$my_chcksums.Add( "' + $filejack + '" , "' + (( Get-FileHash -Path $LocalPath ).SHA256) + '" );' | Out-File -Append $chksum_log )
                ...
                posh-tee "Ending wise365 file if -$file-"
                }
...
# still under development re-adjusted due to website changes on 2018-03-16
		'crystal' {
		...
		'files' { # Added this with tuning on 18-01-27
		. (Join-Path $scriptDir '.\data\Get-LatestKVRT.ps1')
		. (Join-Path $scriptDir '.\data\Get-LatestCCLE.ps1')
		. (Join-Path $scriptDir '.\data\Get-NPEVersion.ps1')
		. (Join-Path $scriptDir '.\data\Get-ADWVersion.ps1')
