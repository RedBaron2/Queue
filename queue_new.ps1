...
<#
	...
	Work on Checksum chk/log pulling Checksum for compare to determine newness
	Work on double check for Tron checksum
	Work on changing checksum in hashtable/file
	Work on Adding all checksum functions being on file
...
function Get-FileFromWeb ( $Url, $filePath, $fileName ) {
	...
	posh-tee "Get-FileFromWeb $LocalPath"
		...
		if (( $fn -match "tweaking" ) -and ($dwnld)) {
		posh-tee "We have detected #green#tweaking# has an #red#update#"
		posh-tee "website_version new -$website_version-"
			$extArgs = @{
			# svnzip_local = "%programdata%\chocolatey\tools\7z.exe x"
			archive = "$LocalPath"
		...
		$Extraction_version = (( Extraction @extArgs ))
			...
			if (( Test-Path $LocalPath )) {
				if ( $website_version -ne $Extraction_version ) {
					posh-tee "This will require the deletion of -$LocalPath-"
					del $LocalPath
...
if (!(test-path $output_dir)) {
	...
	cmd /c 7z x "$archive" -o"$output_dir" "Repair_Windows.exe" -r | out-null
	posh-tee "just ran 7zip"
	# checking for the fileVersion of $output_file
	$fileVersion = [System.Diagnostics.FileVersionInfo]::GetVersionInfo( "${env:temp}\few\Tweaking.com - Windows Repair\Repair_Windows.exe" ).FileVersion
...
$fileVersion = $fileVersion -replace("\.0","") # May need to adjust this for future usage of Extraction beyond tweaking
...
foreach ( $_ in $queue ) {
	...
	posh-tee "We are #Starting $_ Now"
		...
		'wise365' {
		. (Join-Path $scriptDir '.\data\Get-LatestWiseCare.ps1')
